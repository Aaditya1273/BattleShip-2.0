document.addEventListener('DOMContentLoaded', () => {
  ...
  function startMultiPlayer() {
    ...
    startButton.addEventListener('click', () => {
      if(allShipsPlaced) {
        ...
        // Play start sound
        if (window.playSound) window.playSound('turn-change', { volume: 0.7 });
        ...
        // Play error sound
        if (window.playSound) window.playSound('miss', { volume: 0.3 });
      }
    });
  ...
  function startSinglePlayer() {
    ...
    // Initialize turn display to "PLACE SHIPS" mode
    updateTurnIndicator(null, 'setup');

    startButton.addEventListener('click', () => {
      // Play start sound
      if (window.playSound) window.playSound('turn-change', { volume: 0.8 });
  ...
  function rotate() {
    // Play rotation sound
    if (window.playSound) window.playSound('turn-change', { volume: 0.3 });
  ...
  function dragDrop() {
    ...
    if (isHorizontal) {
      ...
      if (!overlapping) {
        ...
        if (window.playSound) window.playSound('ship-placed', { volume: 0.6 });
  ...
  function playGameSingle() {
    ...
    // Initial turn display update
    if (currentPlayer === 'user') {
      updateTurnIndicator('user', 'game');
    } else {
      updateTurnIndicator('computer', 'game');
    }
    ...
    if (currentPlayer === 'computer') {
      ...
      setTimeout(() => {
        ...
        if (tries < maxTries) {
          ...
          if (hit) {
            // Play hit sound
            if (window.playSound) window.playSound('hit', { volume: 0.7 });
            animateElement(userSquares[randomGo], 'shake', 500)
            
            // Count ship hits
            if (userSquares[randomGo].classList.contains('destroyer')) cpuDestroyerCount++
            if (userSquares[randomGo].classList.contains('submarine')) cpuSubmarineCount++
            if (userSquares[randomGo].classList.contains('cruiser')) cpuCruiserCount++
            if (userSquares[randomGo].classList.contains('battleship')) cpuBattleshipCount++
            if (userSquares[randomGo].classList.contains('carrier')) cpuCarrierCount++
          } else {
            // Play miss sound
            if (window.playSound) window.playSound('miss', { volume: 0.5 });
          }
          ...
          if (!isGameOver) {
            ...
            if (window.playSound) window.playSound('turn-change', { volume: 0.4 });
            
            // Enable user clicking after computer's turn
            computerSquares.forEach(square => {
      ...
      // Enable click events on computer's grid
      computerSquares.forEach(square => {
        square.addEventListener('click', function handler(e) {
          if (currentPlayer !== 'user' || isGameOver) return;
          
          if (!square.classList.contains('boom') && !square.classList.contains('miss')) {
            // Disable further clicks until current move is processed
            computerSquares.forEach(sq => {
              sq.style.pointerEvents = 'none';
              sq.removeEventListener('click', handler);
            });
            ...
            if (square.classList.contains('taken')) {
              ...
              // Add hit effect
              animateElement(square, 'shake', 500);
              
              // Update ship hit counter
            ...
            if (!isGameOver) {
              ...
              // Call play again for computer's turn after a delay
              setTimeout(() => {
                playGameSingle();
              }, 1000);
            }
  ...
  function showVictoryScreen() {
    ...
    // Play victory sound
    if (window.playSound) {
      // Stop background sound first to make victory sound more prominent
      if (window._audioSources && window._audioSources['background']) {
        window._audioSources['background'].stop();
      }
      // Play victory sound with higher volume
      window.playSound('game-over', { volume: 0.8 });
    }
    ...
    setTimeout(() => {
      ...
      // Restart background sound at lower volume after delay
      setTimeout(() => {
        if (window.playSound && !isMuted) {
          window.playSound('background', { loop: true, volume: 0.1 });
        }
      }, 3000);
    }, 100);
  }
  ...
  function showDefeatScreen() {
    ...
    if (window.playSound) {
      ...
      // Play defeat sound
      window.playSound('game-over', { volume: 0.6 });
    }
  ...
  function setupSoundEffects() {
    ...
    try {
      ...
      // Sound bank - using both oscillator sounds and audio files
      const sounds = {
        // Sound effects using oscillators (as fallback)
        'hit-fallback': createOscillator(440, 'sawtooth', 0.3),
        'miss-fallback': createOscillator(220, 'sine', 0.2),
        'ship-placed-fallback': createOscillator(660, 'square', 0.15),
        'turn-change-fallback': createOscillator(880, 'triangle', 0.25),
        'game-over-fallback': createOscillator(220, 'sawtooth', 0.5)
      };
      ...
      const soundEffects = [
        { name: 'hit', url: '/sounds/explosion.mp3' },
        { name: 'miss', url: '/sounds/splash.mp3' },
        { name: 'ship-placed', url: '/sounds/ship-placed.mp3' },
        { name: 'turn-change', url: '/sounds/turn-change.mp3' },
        { name: 'game-over', url: '/sounds/game-over.mp3' },
        { name: 'background', url: '/sounds/ocean-waves.mp3' }
      ];
      ...
      function loadSound(name, url) {
        return fetch(url)
          .then(response => {
            if (!response.ok) {
              throw new Error(`Failed to load sound ${name}: ${response.status} ${response.statusText}`);
            }
      ...
      Promise.all(soundEffects.map(sound => loadSound(sound.name, sound.url)))
        .then(() => {
          console.log('All sounds loaded successfully');
          // Start background sound when all sounds are loaded
          playBackgroundSound();
        })
        .catch(error => {
      ...
      // Create oscillator function (for fallbacks)
      function createOscillator(frequency, type, duration) {
        return function() {
          try {
            ...
            console.error("Error playing oscillator sound:", e);
          }
        };
      }
      
      // Play sound function using audio buffer
      function playAudioBuffer(name, options = {}) {
        try {
          if (audioContext.state === 'suspended') {
            audioContext.resume();
          }
          ...
          if (!buffer) {
            console.warn(`Sound ${name} not found in buffer, using fallback if available`);
            if (sounds[`${name}-fallback`]) {
              sounds[`${name}-fallback`]();
            }
            return;
          }
          ...
          if (options.loop) {
            ...
            window._audioSources[name] = source;
          }
      ...
      window.playSound = function(soundName, options) {
        playAudioBuffer(soundName, options);
      };
      ...
      // Add sound effects to hit and miss
      const originalRevealSquare = window.revealSquare;
      if (typeof revealSquare === 'function') {
        window.revealSquare = function(classList) {
          const result = originalRevealSquare.apply(this, arguments);
          
          // Play hit or miss sound based on result
          if (Object.values(classList).includes('boom')) {
      ...
      // Add mute button with improved styling
